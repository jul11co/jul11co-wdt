...
util.inherits(JsonStore, EventEmitter);

JsonStore.prototype.exit = function(err) {
  if (this._save_on_exit) {
...
function exitHandler(code, reason) {
  if (!this._exited) {
    this._exited = true;
    if (!this._readonly && typeof this._dbfile != 'undefined') {
      this.save(this._dbfile);
      // console.log('Data saved to: ' + this._dbfile);
    }
  }
}

JsonStore.prototype.load = function(dbfile) {
  var db = null;
  try {
...
JsonStore.prototype.save = function(dbfile) {
  var err = null;
  try {
...
JsonStore.prototype.toMap = function() {
  return this._db;
}
...
function updateObject(original, update, verbose) {
  if (typeof original == 'object' && typeof update == 'object') {
    for (var prop in update) {
      if (verbose) {
        console.log('[JsonStore]','Update prop "' + prop + '":', 
...
JsonStore.prototype.update = function(key, update, save_to_file) {
  if (typeof this._db[key] == 'object' && typeof update == 'object') {
    updateObject(this._db[key], update);
  } else {
    this._db[key] = update;
  }
  if (save_to_file && !this._readonly && typeof this._dbfile != 'undefined') {
    this.save(this._dbfile);
  }
}

// for array data only
JsonStore.prototype.push = function(key, value, save_to_file) {
...
JsonStore.prototype.delete = function(key, save_to_file) {
